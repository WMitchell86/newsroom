"""M2.1A ArticleRecord contract + deterministic normalization (stdlib only).

Frozen Radar untouched: no imports from sources/rss.py, state/*,
notify/*, poll.py. Missing values stay explicit None — never invented.
"""
...
def make_article_id(url: str, published_at: str | None, headline: str | None) -> str:
    """Legacy M2.1 id (kept for backward compat). Prefer stable_article_id."""
...
def article_to_dict(record: ArticleRecord) -> dict[str, object]:
    ...
    data["categories"] = list(record.categories)
    return data
...
def find_duplicates(records: list[ArticleRecord]) -> dict[str, list[list[str]]]:
    """Inspect duplicate signals; record groups, destroy nothing."""
    ...
    for record in records:
        ...
        if record.post_id:
            by_post.setdefault(record.post_id, []).append(record.article_id)
    ...
    return {
        ...
        "by_post_id": [sorted(g) for g in by_post.values() if len(g) > 1],
...
def build_manifest(
    ...
    *,
    ...
    """Reproducible manifest over chronological published_date."""
    from statistics import median

    failures = failures or []
    dates = sorted(r.published_date for r in records if r.published_date)
    category_counts: dict[str, int] = {}
    author_counts: dict[str, int] = {}
    for record in records:
        for cat in record.categories or ([record.category] if record.category else []):
    ...
    return CorpusManifest(
        ...
        missing_date=sum(1 for r in records if not r.published_date),
