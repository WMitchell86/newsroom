...
* API-backed provider (Brave first); consumer Bing/Google HTML scraping is
  explicitly NOT a production path;
...
def resolve_provider(env=None):
    """Pick the configured provider; missing config is an explicit outcome.
...
def run_search_operation(
    *,
    ...
    """
    provider, status = (provider, SEARCH_OK) if provider is not None else resolve_provider(env=env)
    operation = {
        "topic": topic,
    ...
    if provider is None:
        ...
        operation["reason"] = "няма конфигуриран доставчик на търсене (SEARCH_PROVIDER)"
    ...
    for query in queries:
        ...
        operation["queries"].append({k: v for k, v in outcome.items()})
        if outcome["status"] == SEARCH_OK:
